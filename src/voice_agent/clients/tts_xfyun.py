...
class XFYunTTSClient:
    ...
    def _query_audio_download_url(self, task_id: str, wait_seconds: int | None = None) -> str:
        query_url = self._assemble_auth_url(self.query_path)
    ...
    def synthesize_to_file(self, text: str, output_path: str) -> str:
        ...
        for segment in segments:
            task_id = self._create_task(segment)
            # 文本越长，合成耗时越久；动态增加等待上限，避免长文本误判超时。
            estimated_wait = min(600, max(self.max_wait_seconds, int(len(segment) * 0.6)))
            download_url = self._query_audio_download_url(task_id, wait_seconds=estimated_wait)

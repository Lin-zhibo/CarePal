...
class XFYunASRClient:
    ...
    @staticmethod
    def _a17(existing: str, incoming: str) -> str:
        if not incoming:
            return existing
        if not existing:
            return incoming
    ...
    @staticmethod
    def _a19(wav_path: str) -> tuple[bytes, int, str]:
    ...
    @staticmethod
    def _a23(audio_bytes: bytes) -> bytes:
        data = audio_bytes
    ...
    def _a29(self, mp3_path: str) -> tuple[bytes, int, str]:
        with open(mp3_path, "rb") as f:
            audio_bytes = f.read()
        if not audio_bytes:
            raise RuntimeError("MP3音频内容为空，无法识别")

        audio_bytes = self._a23(audio_bytes)
        if not audio_bytes:
            raise RuntimeError("MP3去除ID3信息后为空，请检查音频文件")

        return audio_bytes, self.mp3_sample_rate, "lame"

    def _a31(self, audio_path: str) -> tuple[bytes, int, str]:
        ext = os.path.splitext(audio_path)[1].lower()
        if ext == ".wav":
            return self._a19(audio_path)
        if ext == ".mp3":
            return self._a29(audio_path)
    ...
    def transcribe_file(self, audio_path: str, language: Optional[str] = None) -> str:
        ...
        audio_bytes, sample_rate, audio_encoding = self._a31(audio_path)
        ...
        def on_message(ws: WebSocketApp, message: str) -> None:
            ...
            if result_text_b64:
                text_obj = self._a11(result_text_b64)
                current_text = self._a13(text_obj)
                ...
                if isinstance(sn, int):
                    ...
                    hypothesis_text = self._a17(hypothesis_text, current_text)

...
class MicAutoRecorder:
    ...
    @staticmethod
    def _l07(chunk: np.ndarray) -> float:
...
def run_live_terminal(
    ...
    while True:
        try:
            ...
            p01 = os.path.join(output_dir, f"mic_{ts}.wav")
            p02 = os.path.join(output_dir, f"reply_{ts}.mp3")

            r01.record_once(p01)
            r02 = pipeline.run_from_audio(
                audio_path=p01,
                history=history,
                output_audio_path=p02,
                language=language,
                stream_to_console=True,
            )
            print(f"[ASR] {r02.user_text}")
            print(f"[TTS] {r02.audio_output_path}")

            if autoplay:
                print("[AUDIO] 播放中...")
                _l11(r02.audio_output_path)
